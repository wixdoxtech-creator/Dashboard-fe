...
const Telegram = () => {
  ...
  const [telegramData, setTelegramData] = useState<SocialData[]>([]);
  ...
  const { data: telegramApiData,
    isLoading: queryLoading,
    isFetching,
   } = useGetSocialDataQuery(
    { email, deviceImei, entity: "telegram" },
    { skip, refetchOnMountOrArgChange: false, refetchOnFocus: false }
  );

  const loading = authLoading || queryLoading || (isFetching && telegramData.length === 0);

 useEffect(() => {
  if(skip) return;
  const result = telegramApiData?.data ?? [];
  setTelegramData(result)
 }, [skip, telegramApiData])
  ...
  const confirmAndDeleteSelected = () => {
    ...
    requestAnimationFrame(() => { void handleDeleteSelected(); });
  };
    ...
    <div className="flex flex-col">
      ...
      <div className="px-4 md:px-8 py-2 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 min-w-0">
          <img src="/telegram.png" alt="Telegram" className="w-8 h-8 sm:w-10 sm:h-10 shrink-0" />
          <h1 className="text-2xl pb-2 sm:text-4xl md:text-5xl font-bold text-blue-500 min-w-0 truncate">
            Telegram
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="hidden sm:flex items-center gap-2">
            ...
            <Button
              ...
              disabled={telegramData.length === 0 || deleting === "all" || authLoading || !isAuthenticated}
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <Tabs defaultValue="chats" className="w-full" value={activeTab} onValueChange={handleTabChange}>
            ...
            <TabsContent value="chats" className="m-0">
              {loading ? (
                ...
                <ChatList data={telegramData} onSelectChat={handleSelectChat} />
              )}
            </TabsContent>
        ...
        <div className={`${isMobileView && !showDetail ? "hidden" : "flex"} flex-1 flex-col`}>
          {isMobileView && showDetail && (
            <div className="p-2 flex items-center bg-blue-200">
              <button
          ...
          <div className="flex-1 p-2 md:p-4">
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 h-full">
              {activeTab === "chats" && (
                <ConversationBox messages={telegramData} selectedUser={selectedUser} />
              )}
              {activeTab === "contacts" && (
      ...
      <AlertDialog open={confirmDeleteSelectedOpen} onOpenChange={setConfirmDeleteSelectedOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {selectedUser ? (
                <>This will permanently delete all chats with <b>{selectedUser}</b>.</>
              ) : (
                <>No user selected.</>
              )}
            </AlertDialogDescription>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting === "selected"}>Cancel</AlertDialogCancel>
            <AlertDialogAction
      ...
      <AlertDialog open={confirmDeleteAllOpen} onOpenChange={setConfirmDeleteAllOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete <b>all</b> Telegram chats for this device.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting === "all"}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              className="bg-red-600 hover:bg-red-700"
