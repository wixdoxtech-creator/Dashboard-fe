...
const Snapchat = () => {
  ...
  const [snapchatData, setSnapchatData] = useState<SocialData[]>([]);
  ...
  const {
    data: snapchatApiData,
    isLoading: queryLoading,
    isFetching,
  } = useGetSocialDataQuery(
    { email, deviceImei, entity: "snapchat" },
    { skip, refetchOnMountOrArgChange: false, refetchOnFocus: false }
  );

  const loading =
    authLoading || queryLoading || (isFetching && snapchatData.length === 0);

  useEffect(() => {
    if (skip) return;
    const result = snapchatApiData?.data ?? [];
    setSnapchatData(result);
  }, [skip, snapchatApiData]);
    ...
    <div className="flex flex-col">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="hidden sm:flex items-center gap-2">
            ...
            <Button
              ...
              disabled={
                snapchatData.length === 0 ||
                deleting === "all" ||
                authLoading ||
                !isAuthenticated
              }
              onClick={() => setConfirmDeleteAllOpen(true)}
              title="Delete all Snapchat chats"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          >
            <TabsList className="grid grid-cols-2 w-full h-[60px] bg-yellow-200">
              <TabsTrigger
                value="chats"
            ...
            <TabsContent value="chats" className="m-0">
              {loading ? (
                ...
                <ChatList data={snapchatData} onSelectChat={handleSelectChat} />
              )}
            </TabsContent>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete all Snapchat chats?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete <b>all</b> Snapchat chats for this
              device.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
