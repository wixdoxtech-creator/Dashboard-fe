  ...
  return (
    <div className="flex flex-col h-[calc(100vh-180px)] bg-gray-100 rounded-lg">
     ...
     <div className="sticky top-0 z-10 px-4 py-3 bg-white/90 backdrop-blur border-b flex items-center gap-3">
        <div className="h-9 w-9 rounded-full bg-red-400 text-white flex items-center justify-center text-sm font-semibold">
            ...
            {filteredMessages.map((msg, idx) => (
              <div key={msg.id ?? `${msg.name ?? "unknown"}-${idx}`} className="flex justify-start">
                <div className="flex items-end space-x-2 max-w-[70%]">
                  {/* <Avatar className="h-9 w-9 bg-red-400 text-white items-center justify-center hidden sm:flex">
