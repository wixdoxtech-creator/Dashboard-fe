        ...
        <ScrollArea className="h-full">
          <div className="space-y-1">
            {chats.map((chat, idx) => {
              ...
              const timeLabel =
                chat.timestamp && !Number.isNaN(new Date(chat.timestamp).getTime())
              ...
              return (
                <div
                  key={key}
                  className="flex items-center space-x-4 px-4 py-3 hover:bg-red-100 cursor-pointer rounded-lg"
                  onClick={() => onSelectChat(safeName)}
                >
                  <Avatar className="h-10 w-10 text-black bg-gray-200 items-center justify-center">
                    <div className="text-sm font-medium">{getInitials(safeName)}</div>
                  </Avatar>
            ...
            {chats.length === 0 && (
              <div className="px-4 py-6 text-center text-gray-500">No chats</div>
            )}
          </div>
        </ScrollArea>
      </div>
