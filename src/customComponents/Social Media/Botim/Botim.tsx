...
const Botim = () => {
  ...
  const [botimData, setBotimData] = useState<SocialData[]>([]);
  ...
  const { data: botimApiData,
    isLoading: queryLoading,
    isFetching,
   } = useGetSocialDataQuery(
    { email, deviceImei, entity: "botim" },
    { skip, refetchOnMountOrArgChange: false, refetchOnFocus: false }
  );

  const loading = authLoading || queryLoading || (isFetching && botimData.length === 0); 

 useEffect(() => {
  if(skip) return;
  const result = botimApiData?.data ?? [];
  setBotimData(result);
 }, [skip, botimApiData])
    ...
    <div className="flex flex-col">
      <div className="px-4 md:px-8 py-2 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <img src="/botim.png" alt="Botim" className="w-10 h-10 rounded-md" />
          ...
          >
            ...
            <TabsContent value="chats" className="m-0">
              {loading ? (
                ...
                <ChatList data={botimData} onSelectChat={handleSelectChat} />
              )}
            </TabsContent>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete all Botim chats?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete <b>all</b> Botim chats for this
              device.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
