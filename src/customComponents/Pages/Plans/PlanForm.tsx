...
// ✅ change here if your route is different
const RENEW_ENDPOINT = "/user/license/renew";
...
export default function PlanForm() {
  ...
  const selectedPlan = location.state?.selectedPlan;
  const passedLicenseId = location.state?.licenseId; // ✅ recommended: pass this while navigating to renew page
  ...
  const finalPrice = Math.max(0, Math.round((currentPrice - discountAmount) * 100) / 100);
  ...
  const getLicenseId = () => {
    return (
      ...
      userDetails?.license?.id || // fallback if you store id directly
      null
    );
  };

  const callRenewApi = async (args: {
    gateway: Gateway;
    orderId?: string | null;
    ...
    // ✅ your backend expects durationDays at top-level, NOT inside payment
    const payload = {
      licenseId: String(licenseId),
      email: String(email),
      durationDays: Number(durationDays),
      amountPaid: Number(finalPrice || 0),
      subtotal: Number(finalPrice || 0),
      gst: 0,
    ...
    await api.post(RENEW_ENDPOINT, payload);
    customToast.info("Payment Successful. License Renewed/Extended!");
    setTimeout(() => navigate("/"), 1500);
  };
    ...
    const options = {
      ...
      handler: async (response: any) => {
        ...
        try {
          ...
          customToast.error("Payment verified failed. Please contact support.");
        }
      },
  ...
  const payWithCashfree = async () => {
    ...
    try {
      ...
      if (verifyRes.ok && verify?.status === "success") {
        const cfPaymentId =
          verify?.paymentId || verify?.cf_payment_id || verify?.reference_id || orderId;
  ...
  const durationLabel = durationOptions.find((opt) => opt.value === selectedDuration)?.label;
              ...
              <div className="flex items-center justify-between mb-6 p-4 bg-green-200/30 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Renewal Amount • {durationDays} days
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-semibold text-green-600">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          >
            ...
            <label className="flex items-center justify-between rounded-lg border p-3 cursor-pointer">
              ...
              <img src="/PayPal.svg.png" alt="PayPal" className="h-8 opacity-90" />
            </label>
            ...
            <label className="flex items-center justify-between rounded-lg border p-3 cursor-pointer">
              ...
              <img src="/cashfree1.png" alt="Cashfree" className="h-8 opacity-90" />
            </label>
          </RadioGroup>
